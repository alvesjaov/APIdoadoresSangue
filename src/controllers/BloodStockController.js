...
async function countAndUpdateBloodTypes(request, response) {
    try {
        ...
        // Conta o número de doações para cada tipo sanguíneo cuja data de validade ainda não passou
        const bloodTypeCounts = await Donor.aggregate([
            ...
            { $group: { _id: '$bloodType', count: { $sum: { $size: "$donationHistory" } } } }, // Agrupa os doadores por tipo sanguíneo e conta o número de doações para cada grupo
