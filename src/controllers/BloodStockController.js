// Importando os modelos Donor e Stock
...
async function countAndUpdateBloodTypes(request, response) {
    try {
        ...
        await Stock.updateOne({}, allCounts, { upsert: true }); // Atualiza um documento na coleção Stock com as novas contagens. Se nenhum documento existir, cria um novo (upsert: true).
        ...
        response.status(500).send('Ocorreu um erro ao contar e atualizar os tipos sanguíneos. Por favor, tente novamente.', error.message);
    }
}
